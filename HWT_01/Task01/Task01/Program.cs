    ...
    class Program//todo pn ты, видимо, неправильно понял правила оформления: для каждой домашки 1 солюшн, в котором столько проектов, сколько заданий в домашке. Переделай согласно требованиям к оформлению.
	{
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                ...
                if (belong == true)
                {
                    Console.WriteLine("Точка ({0};{1}) принадлежит фигуре [{2}].", x, y, letter);//todo pn лучше условие в WriteLine запихнуть, чтобы не дублировать большую часть сообщения в двух местах (через оператор "? :").
				}
                else
                {
                    if (belong == false)
                    {
                        Console.WriteLine("Точка ({0};{1}) не принадлежит фигуре [{2}].", x, y, letter);
                    }
                }
                Console.ReadKey();
            }
